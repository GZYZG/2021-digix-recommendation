...
class GatingNetwork(nn.Module):
    ...
    def __init__(self,input_units, units, num_experts,selectors):
        ...
        self.experts_shared = nn.ModuleList([Expert_shared(input_units, units) for i in range(num_experts)])
        self.experts_task1 = nn.ModuleList([Expert_task1(input_units, units) for i in range(num_experts)])
        self.experts_task2 = nn.ModuleList([Expert_task2(input_units, units) for i in range(num_experts)])
...
class PLE2(nn.Module):
    def __init__(self, num_feature, num_experts, num_tasks, units, towers_hidden, selectors):
        # 0 8 2 16
        super(PLE2,self).__init__()
        ...
        self.gate1 = GatingNetwork(self.num_feature, self.units, self.num_experts, self.selectors)
        self.gate2 = GatingNetwork(self.units, self.units, self.num_experts, self.selectors)
 
        self.towers = nn.ModuleList([Tower(self.units, 1, self.towers_hidden) for i in range(self.num_tasks)])
